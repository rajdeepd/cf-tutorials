$(document).ready(function () {
   ...
   //Add Expense - Event Handler declaration.
	...
	$("button.edit-expense-btn").on("click", function () {
		   $.post('/html5expense/expenses/'+$("input#expenseload").val(), {_method:"PUT", id : $("input#expenseload").val(), description : $(".textbox-a").val(), expenseTypeId : $("#expenseTypeId").val(), amount : $("input#amount").val()} , function(data) {
			   $("a[data-tab=myExpenseContainer]").click();
		   });
	});
	...
	$("table#approvalTable").on("change","select.test", function (){
		...
		$.getJSON('/html5expense/expenses/'+$.trim(self.parents("tr").children("td.expenseid").text()) +'/state/'+$(this).val(), function(jsonResponse) {
	...
	$("table#expenseTable").on("click","a.bg-cross-b", function(evt){
		   ...
		   console.log("delete 2 url"+'/html5expense/expenses/'+self.attr('id')+'/');
		    $.post('/html5expense/expenses/'+self.attr('id')+'/', {_method:'DELETE'}, function() {
				$("a[data-tab=myExpenseContainer]").click();
			});	   
    });
	...
	var pageselectCallback =  function (page_index, jq, opt){
		...
		for(var i=page_index*items_per_page;i<max_elem;i++)
		{
			newcontent += '<tr class="tr-odd"><td>'+ rowsArr[i].id +'</td> <td> <h5 class="label-4">' + new Date(parseInt(rowsArr[i].expenseDate)) +
		      ...
		      rowsArr[i].description + '</h5></td><td><h5 class="label-4"><i class="alt-a label-5 blue"><img class="right-a-img" src="/html5expense/resources/images/question-a.png ">' + 
		      rowsArr[i].state +'</i><i class="alt-b label-6"><img class="question-a-img" src="/html5expense/resources/images/question-a.png" >' + 
		      rowsArr[i].state + '<a href="#" id=' + rowsArr[i].id + ' class="bg-edit-b"> <img class="clip-b-img" src="/html5expense/resources/images/edit-a.png"> </a>'+
		      '<a href="#" id=' + rowsArr[i].id + ' class="bg-cross-b"><img class="cross-b-img" src="/html5expense/resources/images/cross-b.png"> </a></i></h5></td></tr>"';
		}
