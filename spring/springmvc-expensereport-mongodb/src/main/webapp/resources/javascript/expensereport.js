$(document).ready(function () {
   ...
   //Add Expense - Event Handler declaration.
	...
	var loadApprovalPendingExpenses = function (){
		$.getJSON(utils.url('/expenses/approvals'), function(jsonResponse) {
			   ...
			   $.each(jsonResponse, function () {
			      var self = this;
			      expenseList += '<tr class="tr-odd"><td class="expensedate"><h5 class="label-4">' + new Date(parseInt(self.expenseDate)) + '</h5></td><td class="expenseid"> <h5 class="label-4">' + self.expenseId + '</h5></td><td class="expensedescription"><h5 class="label-4">' + self.description + '</h5></td><td class="expensestate"><select id="action_1" class="test" name="1" style="font-size: 11px;height: 25px;width: 120px;"><option selected="selected" value="Pending">PENDING</option><option value="APPROVED">Approve</option><option value="REJECTED">Reject</option></select></td></tr>"';    
			    });
	...
	$("table#expenseTable").on("click","a.bg-edit-b", function(evt){
		   ...
		   var self = $(this);
				$.getJSON(utils.url('/expenses/'+self.attr('id')), function(jsonResponse) {
					...
					$("input#expenseload").val(jsonResponse.expenseId);
	...
	var pageselectCallback =  function (page_index, jq, opt){
		...
		{
			newcontent += '<tr class="tr-odd"><td>'+ rowsArr[i].expenseId +'</td> <td> <h5 class="label-4">' + new Date(parseInt(rowsArr[i].expenseDate)) +
		      ...
		      rowsArr[i].state + '<a href="#" id=' + rowsArr[i].expenseId + ' class="bg-edit-b"> <img class="clip-b-img" src="'+ utils.url('/resources/images/edit-a.png')+'"> </a>'+
		      '<a href="#" id=' + rowsArr[i].expenseId + ' class="bg-cross-b"><img class="cross-b-img" src="'+ utils.url('/resources/images/cross-b.png')+'"> </a></i></h5></td></tr>"';
		}
