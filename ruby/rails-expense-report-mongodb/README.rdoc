== Expense Report Application with Rails and MongoDB

This application is a sample application which is developed in Ruby version 1.9.2-p180 and Rails version 3.2.9. This is using MongoDB as the backend database. And it is using the <tt>mongoid</tt> as the ORM to interact with MongoDB. Below are the steps which will guide you to set up the application locally as well as on CloudFoundry.

== Prerequisite

* Ruby 1.9.2-p180 should be installed on your machine via <tt>rvm</tt>.
* MongoDB should be installed on your machine. Follow this for instructions: http://www.mongodb.org/display/DOCS/Quickstart
...
* Go to the app folder. Do this by running the below command in terminal.
    ...
    cd cf-tutorials/ruby/rails-expense-report-mongodb

* Install the gems specified in <tt>Gemfile</tt> by running following command in terminal.
  
    bundle install

* Run the below step to populate the data which is in <tt>db/seeds.rb</tt>.
...
* Install the above gem by running <tt>bundle install</tt> and then run the above command for populating the database as mentioned in above step.
